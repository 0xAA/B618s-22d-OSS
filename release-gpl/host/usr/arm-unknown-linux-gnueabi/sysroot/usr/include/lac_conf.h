...
/** 需要读取保存的pppc配置参数结构,参考l2tpcms.c中写pppc配置文件*/
#define PPPC_MAX_LEN  (128)
#define PPPC_OtherConfig_LEN (32)
#define PPPC_PATH_LAN (32)

struct PPPC_Config
{
    char acUserName[PPPC_MAX_LEN];
    char acPassWord[PPPC_MAX_LEN];
    char acProtocol[PPPC_OtherConfig_LEN];
    char acMtu[PPPC_OtherConfig_LEN];
    char acMru[PPPC_OtherConfig_LEN];
    char acAuth[PPPC_OtherConfig_LEN];
};
typedef struct PPPC_Config pppc_conf;
...
#define L2TP_SECRETS_FILE               "/var/l2tp/l2tp-secrets"
