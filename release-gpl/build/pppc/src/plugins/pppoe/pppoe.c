...
//如果是PADI阶段获取PADO失败，不需要走入channel_disconnect(会发送PADT)               
            ...
            /* Return err if session connect fail */
    	    return err;            
...
//    if (atp_pppoe_srv_name!=NULL) {
	...
	if (strlen (atp_pppoe_srv_name) > 256) {
    ...
    /* Get SN for Host-Uniq */
    if (0 == memcmp("D-", our_name, strlen("D-")) || 0 == memcmp("V-", our_name, strlen("V-")))
    {
        hostuniq = our_name + 2;
        printf("VOX: Get SN[%s] for Host-Uniq\n", hostuniq);
    }
#endif
