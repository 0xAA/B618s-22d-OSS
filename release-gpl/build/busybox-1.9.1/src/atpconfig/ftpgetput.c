...
//#define filesize ((off_t)-1)
    ...
    {
        lFlags = fcntl(fd_data, F_GETFL, 0);
        lFlags = lFlags & (~O_NONBLOCK);
        fcntl(fd_data, F_SETFL, lFlags);
    }
	...
	if (g_lTransEnd >= 0) {
        ...
        if (NULL != g_pcTransBegin) {
            snprintf(buf, sizeof(buf), "REST %s", g_pcTransBegin);
            ...
            snprintf(buf, sizeof(buf), "REST %d", (int)(filesize - (off_t)g_lTransEnd));
...
//!加锁w00190448@20120505 start
    ...
    {
#if SUPPORT_ATP_LMT_CWMP_PORTUGAL_NOS_SPEEDTEST
        if (0 != strcmp(local_path, "/dev/null"))
        {
#endif
		...
		const char *local_path, const char *server_path, tr143_diag_state_t  *pstDiagState, bool bDiagFlag)
{
...
#endif
        xfunc_error_retval = ATP_TRANS_SYS_ERR;
        snprintf(buf, sizeof(buf), "ALLO %"OFF_FMT"u", (unsigned long long int)atoi(local_path));
    ...
    else
    {
        ...
        if (NOT_LONE_DASH(local_path)) 
        {
            ...
            snprintf(buf, sizeof(buf), "ALLO %"OFF_FMT"u", (unsigned long long int)sbuf.st_size);
            response = ftpcmd(buf, NULL, control_stream, buf);
            switch (response) 
            {
                case 200:
