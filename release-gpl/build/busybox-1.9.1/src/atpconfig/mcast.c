...
#define USER_UPG_FILE               "/var/userupg"
#define USER_UPG_FLAG               77
#define MULTIPKT_TAG_OFFSET         20         
#define MULTI_UPGRADE_PKT_TAG       "fnt-HGW" 
#define MULTI_UPGRADE_PKT_NUM       15   
...
int     upg_mode = 0;
static VOS_VOID set_flag_to_userupg(unsigned char flag)
{
    FILE *fp = NULL;
    fp = fopen(USER_UPG_FILE, "w+");
    if(NULL != fp)
    {
        fprintf(fp, "%d", flag);
        fclose(fp);
...
static int check_packet(char *acRecvBuf)
{
    char msg[] = MULTI_UPGRADE_PKT_TAG;   
    
    if (0 != memcmp(acRecvBuf + MULTIPKT_TAG_OFFSET, MULTI_UPGRADE_PKT_TAG, strlen(msg)))
    {
         printf("compare fnt-HGW fail ");
         return 0;
    }
    printf("check fnt-HGW OK !");
    return 1;       
}
...
#ifdef SUPPORT_ATP_MULTIBOARD_UPG
int g_fd = -1;
#endif
                                             ...
                                             const char *localFile)
{
    ...
    // Open local file for write, slave board is write to flash file directly
#ifdef SUPPORT_ATP_MULTIBOARD_UPG
    if (-1 == g_fd)
    {
        ...
        g_lMcastLen     = 0;
        g_fd = open(localFile, O_RDWR | O_CREAT);
        if ((g_fd <= 0) || ((0 < g_lMaxTrans) && (ulTmpInt > g_lMaxTrans))) {
	...
	if ((ulCurSeqNum + len) > g_ulTotalLen) {
		printf("Seq num %d too large\n", ulCurSeqNum);
		return 1;
	}
	DBG_PRINT("Store seq num %d len: %d total len: %d\n",
	  			ulCurSeqNum, len, g_lMcastLen);

    lseek(g_fd, ulCurSeqNum, SEEK_SET);
    write(g_fd, pstPkg, len);
#else
                                             ...
                                             const char *localFile)
{
    ...
    /* wait for multiboard communication ok */
#ifdef SUPPORT_ATP_MULTIBOARD_UPG
    while(iWaitTimes > 0)
    {
        memset(acFlag, 0, sizeof(acFlag));
        pfFile = fopen("/var/multiboard/intf_status", "rb");
        if (NULL != pfFile)
        {
            fread(acFlag, 1, 1, pfFile);
            if (1 == atoi(acFlag))
            {
                printf("MultiBoard communication is ok\n");
                break;
            }
            fclose(pfFile);
        }
    ...
    snprintf(acArg, sizeof(acArg), "upg -a %s -t '1 Firmware Upgrade Image' -b -c mcast -l %s", localFile, g_pcLocalIP);
    DBG_PRINT("Do upg command to mcast upgrade [%s].\n", acArg);
    system(acArg);
                           ...
                           const char *pcRemoteIp, int port, int mode, int lTimeout)
{
    ...
    while (1)
    {
        ...
        if((1 == upg_mode) && (lBytesRecvd > 0))
        {
             if(1 == check_packet(acRecvBuf))
             {
                 printf("write flag \n");
                 set_flag_to_userupg(USER_UPG_FLAG);
             }
             break;
        }
...
int mcast_main(int argc, char **argv)
{
    ...
    g_lMcastSeqNum          = 0;
    g_lMcastLen             = 0;
#ifndef SUPPORT_ATP_MULTIBOARD_UPG
    g_pucMcastBuf           = NULL;
#endif
    g_pcLocalIP             = NULL;
    pcTimeout               = NULL;
    ...
    opt = getopt32(argv, "gsdu:p:r:l:P:B:A:b:e:m:"ZIP_OPT_CHAR"zf:t:C:",
                         &dummy,        // Username
                         &dummy,        // Password
                         &dummy,        // Remote path
...
#undef ZIP_OPT_CHAR
    ...
    if (opt & TRANS_TOOL_OPT_CUSTOM_UPG) {    
		upg_mode = 1;     
	}
