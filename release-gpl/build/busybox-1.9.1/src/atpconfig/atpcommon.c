...
int xbind_connect(const len_and_sockaddr *lsa, const char *ip)
{
    ...
    while (1)
    {
        ...
        stTimeout.tv_sec  = ATP_PACK_TIMEOUT_D;     //连接超时30秒
        stTimeout.tv_usec = 0;

        lRet = select((fd + 1), &hRdSet, &hWrSet, 0, &stTimeout);
        if (lRet < 0)
        {
            if (EINTR == errno)
            {
        ...
        {
            if ((FD_ISSET(fd, &hRdSet)) || (FD_ISSET(fd, &hWrSet)))
            {
                lRet = sizeof(lError);
                if ((getsockopt(fd, SOL_SOCKET, SO_ERROR, (void *)(&lError), (socklen_t *)(&lRet)) < 0) || (0 != lError))
                {
                    ulRetVal = ATP_TRANS_TIMEOUT;
                }
            }
...
int ATP_TransHostname2IP(char * hostname,char * buffer,int len,int * pIPflag)
{
    ...
    {
        memset(buffer,0,len);
        snprintf(buffer, len, "%s", hostname);
        *pIPflag = ATP_IPV6_IP;
        return 1;
    }
    ...
    {  
        ...
        memset(buffer,0,len);
        snprintf(buffer, len, "%s", inet_ntoa(sin.sin_addr));
        *pIPflag = ATP_IPV4_IP;
        return 1;
    }
}
...
void ATP_UTIL_GetIfcStatInfo(char *pszIfcName, ATP_UTIL_INTERFACE_STAT_ST *pstState)
{
    ...
    while (NULL != fgets(acLine, sizeof(acLine), fs) )
    {
        ...
        if ((NULL != pszChar) && isdigit(*pszChar) )
        {
            ...
            *pszChar = ' ';
            ++pszChar;
            lIndx = (int)((char *)pszChar - (char *)acLine);
            snprintf(pszChar, sizeof(acLine) - lIndx, "%s", acBuf);   
        }

        pszChar = strstr(acLine, pszIfcName);
        /* 如果接口被找到，则存储统计值 */
        if ((NULL != pszChar) && (':' == *(pszChar + strlen(pszIfcName))))
        {
