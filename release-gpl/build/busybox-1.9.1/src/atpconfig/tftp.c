...
#define CLIENT_PORT_UNAVAILABLE          (-1)
#define TFTP_BUF_TEMP_LEN_16             (16)
#define TFTP_BUF_TEMP_LEN_256            (256)

static void sbb_tftp_add_delete_iptables_for_port(int client_port, bool isAddIptables)
{
    char bufCmd[TFTP_BUF_TEMP_LEN_256];
    char bufOption[TFTP_BUF_TEMP_LEN_16];
    ...
    memset(bufOption, 0, sizeof(bufOption));
    if (isAddIptables) /* 添加iptables规则 */
    {
        snprintf(bufOption, sizeof(bufOption), "%s", "-A");
    }
    else    /* 删除iptables规则 */
    {
        snprintf(bufOption, sizeof(bufOption), "%s", "-D");
    }

    /* 执行iptables命令，由于tftp服务器会使用不同的UDP端口回数据报文，因此，需要临时打开端口的防火墙规则，允许报文接收 */
    memset(bufCmd, 0, sizeof(bufCmd));
    snprintf(bufCmd, sizeof(bufCmd), "iptables -t nat %s PRE_SERVICE -p udp --dport %d -j ACCEPT", bufOption, client_port);
    system(bufCmd);
        
    memset(bufCmd, 0, sizeof(bufCmd));
    snprintf(bufCmd, sizeof(bufCmd), "iptables %s INPUT_SERVICE -p udp --dport %d -j ACCEPT", bufOption, client_port);
    system(bufCmd);
}
#endif
...
#ifdef SUPPORT_ATP_CPE_SBB
    /* tftp使用固定端口，被前面的应用给占用了，导致tftp不通，air3定制上修正此问题，将tftp端口改为随机端口，主线不处理 */
    int client_port = CLIENT_PORT_UNAVAILABLE;
    socklen_t lenSockAddr = 0;
...
#ifdef SUPPORT_ATP_CPE_SBB
    /* tftp使用固定端口，被前面的应用给占用了，导致tftp不通，air3定制上修正此问题，将tftp端口改为随机端口，主线不处理 */
    socketfd = create_and_bind_dgram_or_die(g_pcLocalIP, 0);
    if (socketfd < 0)   /* 创建or 绑定socket error */
    {
        bb_error_msg("create socket or bind error!!");
        client_port = CLIENT_PORT_UNAVAILABLE;
        goto ret;
    }

    lenSockAddr = sizeof(sin);
    memset(&sin, 0, sizeof(sin));
    if (-1 == getsockname(socketfd, (struct sockaddr *)&sin, &lenSockAddr))
    {
        bb_error_msg("getsockname error!!");
        client_port = CLIENT_PORT_UNAVAILABLE;
        goto ret;
    }

    client_port = ntohs(sin.sin_port);
    sbb_tftp_add_delete_iptables_for_port(client_port, true);  
#else
...
#ifdef SUPPORT_ATP_CPE_SBB
    sbb_tftp_add_delete_iptables_for_port(client_port, false);  
#endif
