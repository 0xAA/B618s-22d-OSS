...
#define WAN_ALG_LISTEN_PORT 56005
...
extern int open_logcmd;
extern void print_url_map();
...
#else
   sip_udp_wan_socket = sip_udp_lan_socket;
#endif
...
#endif
{
...
#if defined (TOS_RCV) &&defined(SUPPORT_ALG_PRIVILEGE_USER)
   char tos_temp = 0;
   char ctl_buf[64];
   struct msghdr msg;
   struct iovec iov;
   ...
   for (i=0; i<URLMAP_SIZE; i++) {
      ...
      if (urlmap[i].wan_socket_fd) {
         /* add to FD set */
         FD_SET(urlmap[i].wan_socket_fd, &fdset);
         DEBUGC(DBCLASS_BABBLE,"urlmap[%d].wan_socket_fd = %d is FD_SET, urlmap[i].wan_socket_fd = %d", i, urlmap[i].wan_socket_fd, urlmap[i].wan_socket_fd);
         if (urlmap[i].wan_socket_fd > highest_fd) {
            highest_fd = urlmap[i].wan_socket_fd;
         }
      } /* if fd > 0 */
   }
#else
         ...
         *protocol = PROTO_TCP;
         ...
         if ((length > 2) && 
            ...
            if (length > 0) {
               ...
               sip_tcp_cache[i].traffic_ts = stTm.tv_sec;
...
#endif
{
   ...
   if (protocol == PROTO_UDP) {
      ...
      DEBUGC(DBCLASS_NET,"send UDP packet to %s: %i, direct = %d", utils_inet_ntoa(addr),port, direct);
...
#endif
         DEBUGC(DBCLASS_NET,"send UDP packet to sip_udp_lan_socket");
      ...
      {
         int tmp_sip_udp_wan_socket = 0;
#ifdef SUPPORT_ATP_SIPROXYD
         tmp_sip_udp_wan_socket = urlmap[index].wan_socket_fd;
#else
         tmp_sip_udp_wan_socket = sip_udp_wan_socket;
#endif
         /* 合入TELMEX的打mark的需求*/
#ifdef SUPPORT_ALG_PRIVILEGE_USER
        ...
        lATPTos = lATPTos;
         ...
         sipsock_setopt(tmp_sip_udp_wan_socket, t_online, direct, tos);
#else
        sipsock_setopt(tmp_sip_udp_wan_socket, lATPOnlineState, direct, lATPTos);
#endif
#ifndef SUPPORT_ALG_PRIVILEGE_USER
         /* 触发按需拨号打的mark */
         sipsock_setmark(tmp_sip_udp_wan_socket);
#endif
         sts = sendto(tmp_sip_udp_wan_socket, buffer, size, 0,
                      (const struct sockaddr *)&dst_addr,
                      (socklen_t)sizeof(dst_addr));
         DEBUGC(DBCLASS_NET,"in sipsock_send tmp_sip_udp_wan_socket = %d, sts = %d", tmp_sip_udp_wan_socket, sts);
...
int get_wan_socket(char *ifname, int *plWanPort)
{
    ...
    //从默认配置的56005开始尝试端口号，防止与别的服务冲突
    if ((0 == iWanPort) || (iWanPort >= (WAN_ALG_LISTEN_PORT + URLMAP_SIZE)))
    {
        iWanPort = WAN_ALG_LISTEN_PORT; 
    }
    
    memset(&wanipaddr, 0, sizeof(wanipaddr));
    if (NULL == ifname)
    {
        ifname = configuration.outbound_if;
    }
    ...
    {
        DEBUGC(DBCLASS_SOCK, "urlmap[%d].wan_port = %d", j, urlmap[j].wan_port);
        if ((1 == urlmap[j].active) && (urlmap[j].wan_port == iWanPort))
        {
            break;
        }
    }
    /* 当与已有的被占用的端口没有冲突时，尝试绑定 */
    if (j >= URLMAP_SIZE)
    {
        iRetFd = sockbind(wanipaddr, iWanPort, PROTO_UDP, 1, SOCK_CREATE_BY_SIP);
    }
    else
    {
        iRetFd = 0;
    ...
    if (0 == iRetFd)
    {
        for (i = 0; i < URLMAP_SIZE; i++)
        {
            ...
            for (j = 0; j < URLMAP_SIZE; j++)
            {
                if ((1 == urlmap[j].active) && (urlmap[j].wan_port == iWanPort))
                {
                    DEBUGC(DBCLASS_SOCK, "conflict iWanPort = %d", iWanPort);
                    break;
                }
            }
            /* 当与已有的被占用的端口冲突时，跳过该端口 */
            if (j < URLMAP_SIZE)
            {
                continue;
            }
