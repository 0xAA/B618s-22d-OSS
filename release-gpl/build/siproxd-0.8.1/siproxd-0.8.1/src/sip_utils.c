...
static int t_online_url(osip_uri_t *url)
{
	if (url != 0 && url->host != 0) {
		...
		if (pstr != 0) {
			pstr++;
			if (!strcasecmp(pstr, PRIVILEGE_USER_DOMAIN)) {
				//printf("siproxd: is_t_online=true\n"); 
				return 1;
			}
			if (!strcasecmp(url->host, PRIVILEGE_USER_DOMAIN)) {
				//printf("siproxd: is_t_online=true\n"); 
				return 1;
			}
		}
...
#ifdef SUPPORT_ALG_PRIVILEGE_USER
   sipsock_send(addr, port, ticket->protocol, buffer, buflen, 0, 0, 0, ticket->index);
#else
   sipsock_send(addr, port, ticket->protocol, buffer, buflen, 0, ticket->index);
#endif
 ...
 *	STS_FAILURE on error
 */
int sip_rewrite_via (sip_ticket_t *ticket, int direction) {
    osip_via_t *via;
    int sts;
    char chost[20] = {0};
    char cport[10] = {0};
    char cmatch_host[20] = {0};
    char cmatch_port[10] = {0};
    struct in_addr addr;
    osip_generic_param_t *rport=NULL;
    osip_generic_param_t *received=NULL;
    osip_generic_param_t *u_param;
    osip_generic_param_t *alp=NULL;
    char tmp[6];
    int i = 0;
    DEBUGC(DBCLASS_PROXY, "in sip_rewrite_via direction =%d",direction);

    if (direction == DIR_OUTGOING)
    {
       snprintf(cmatch_host, sizeof(cmatch_host), "%s", utils_inet_ntoa(urlmap[ticket->index].from.sin_addr));
       snprintf(cmatch_port, sizeof(cmatch_port), "%d", ntohs(urlmap[ticket->index].from.sin_port));
       if (get_interface_ip(IF_OUTBOUND, &addr) != STS_SUCCESS) {
          return STS_FAILURE;
       }
       snprintf(chost, sizeof(chost), "%s", utils_inet_ntoa(addr));
       snprintf(cport, sizeof(cport), "%d", urlmap[ticket->index].wan_port);
    }
    else
    {
        if (get_interface_ip(IF_OUTBOUND, &addr) != STS_SUCCESS) {
        ...
        snprintf(cmatch_host, sizeof(cmatch_host), "%s", utils_inet_ntoa(addr));
        snprintf(cmatch_port, sizeof(cmatch_port), "%d", urlmap[ticket->index].wan_port);
        snprintf(chost, sizeof(chost), "%s", utils_inet_ntoa(urlmap[ticket->index].from.sin_addr));
        snprintf(cport, sizeof(cport), "%d", ntohs(urlmap[ticket->index].from.sin_port));
    }
    ...
    if (via->port)
    {
        DEBUGC(DBCLASS_PROXY, "in sip_rewrite_via atoi(via->port) =%d",atoi(via->port));
    }
    DEBUGC(DBCLASS_PROXY, "in sip_rewrite_via configuration.sip_listen_port = %d",configuration.sip_listen_port);

    if (0 == strncmp(via->host, cmatch_host, strlen(via->host)))
    {
       DEBUGC(DBCLASS_PROXY, "rewriting via header %s -> %s",
                 (via->host)? via->host : "*NULL*",
                 chost);
       osip_free(via->port);
       via->port = NULL;
       osip_free(via->host);
       ...
       via->host = osip_strdup (chost);
       if (via->host == NULL) {
          ERROR("I'm trying to rewrite a via but fail chost = %s", chost);
          return STS_FAILURE;
       }
       via->port = osip_strdup (cport);
       if (via->port == NULL) {
          ERROR("I'm trying to rewrite a via but fail cport = %s", cport);
          return STS_FAILURE;
       }
       
       osip_via_param_get_byname (via, "rport", &rport);
       if (rport && rport->gvalue)
       {
          if (0 == strncmp(rport->gvalue, cmatch_port, strlen(rport->gvalue)))
          {
             osip_free(rport->gvalue);
             rport->gvalue = osip_strdup (cport);
             if (rport->gvalue == NULL) {
                ERROR("I'm trying to rewrite a via but fail rport->gvalue cport = %s", cport);
                return STS_FAILURE;
             }
          }
       }
       ...
       {
          if (0 == strncmp(received->gvalue, cmatch_host, strlen(received->gvalue)))
          {
             osip_free(received->gvalue);
             received->gvalue = osip_strdup (chost);
             if (received->gvalue == NULL) {
                ERROR("I'm trying to rewrite a via but fail received->gvalue cport = %s", cport);
                return STS_FAILURE;
             }
          }
       }
    }
    //将via中的port修改为网关监听的端口，防止软交换之类的从5061等非监听端口发出invite请求，via中带的port也是5061，
    //导致response会往5061发，没有被网关监听到。因此将外部过来的请求的via的port强制修改为网关的监听端口，同时在alg参数中记录该原始port
    else if ((direction == DIR_INCOMING) && (MSG_IS_REQUEST(ticket->sipmsg)) && 
        (((via->port) && (atoi(via->port) != configuration.sip_listen_port)) ||
        ((via->port == NULL) && (5060 != configuration.sip_listen_port))))
    {
#if 1
        //先添加alg参数，不然会被free掉
        if (via->port)
        {
            snprintf(tmp, sizeof(tmp), "%s", via->port);
        }
        else
        {
            snprintf(tmp, sizeof(tmp), "%s", "5060");
        }
        DEBUGC(DBCLASS_PROXY, "port tmp = %s",tmp);
        osip_via_param_add(via,osip_strdup("alg"),osip_strdup(tmp));

        //修改port参数
        osip_free(via->port);
        snprintf(tmp, sizeof(tmp), "%i", configuration.sip_listen_port);
        DEBUGC(DBCLASS_PROXY, "sip_listen_port tmp = %s",tmp);
        via->port = osip_strdup (tmp);
        
#else
        char acCmd[256];
        snprintf(acCmd,sizeof(acCmd), "iptables -t nat -%c PRE_DONAT -i %s -p udp -d %s --dport %s -j REDIRECT --to-ports %d 2>/dev/null",
            'A', "br+", via->host, via->port, configuration.sip_listen_port);
        system(acCmd);
#endif
    }
    //对应的在收到alp参数时将alg替换进port参数，同时删除alp参数
    else if ((direction == DIR_OUTGOING) && (0 == osip_via_param_get_byname (via, "alg", &alp)))
    {
        if (alp && alp->gvalue)
        {
            //将port参数改成alp参数的值
            osip_free(via->port);
            snprintf(tmp, sizeof(tmp), "%s", alp->gvalue);
            via->port = osip_strdup (tmp);

            //删除alp参数
            i = 0;
            while (!osip_list_eol (&via->via_params, i)) {
               u_param = (osip_generic_param_t *) osip_list_get (&via->via_params, i);
               if (u_param == alp)
               {
                   DEBUGC(DBCLASS_PROXY, "before i = %d",i);
                   osip_list_remove (&via->via_params, i);
                   osip_uri_param_free (u_param);
                   DEBUGC(DBCLASS_PROXY, "after i = %d",i);
                   break;
               }
               i++;
            }
    ...
    DEBUGC(DBCLASS_PROXY, "exit sip_rewrite_via");
...
int sip_rewrite_contact (sip_ticket_t *ticket, int direction) {
   ...
   for (j=0; contact != NULL; j++) {
      ...
      DEBUGC(DBCLASS_PROXY, "rewriting Contact header %s@%s -> %s, cmatch_host = %s",
                   (contact->url->username)? contact->url->username : "*NULL*",
                   (contact->url->host)? contact->url->host : "*NULL*",
                   chost, cmatch_host);
#ifdef SUPPORT_ATP_SIPROXYD
      
      char *contact_port = (contact->url->port ? contact->url->port : "5060");

      if (0 == strncmp(contact->url->host, cmatch_host, strlen(contact->url->host))
          && ( 0 == strncmp(contact_port, cmatch_port, strlen(contact_port))
             || (1 == osip_list_size(&sip_msg->contacts))))
      {
         ...
         contact->url->host = osip_strdup (chost);
         if (contact->url->host == NULL) {
            ERROR("I'm trying to rewrite a contact but fail chost = %s", chost);
            return STS_FAILURE;
         }
         contact->url->port = osip_strdup (cport);
         if (contact->url->port == NULL) {
            ERROR("I'm trying to rewrite a contact but fail cport = %s", cport);
            return STS_FAILURE;
         }
         replaced=1;
         ...
         if (direction == DIR_INCOMING)
         {
             ...
             osip_list_add(&(sip_msg->contacts),contact,0);
         }
#endif
...
unsigned int sip_get_top_via_port(sip_ticket_t *ticket)
{
    osip_via_t     *top_via   = 0;
    
    top_via = (osip_via_t *)osip_list_get(&(ticket->sipmsg->vias), 0);
    if (top_via && top_via->port)
    {
        return atoi(top_via->port);
    }
