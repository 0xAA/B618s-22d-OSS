...
int proxy_request (sip_ticket_t *ticket) {
   ...
   if (ticket->readfd != sip_udp_lan_socket)
   {
       for (i = 0; i < URLMAP_SIZE; i++)
       {
           if ((ticket->readfd == urlmap[i].wan_socket_fd) && (1 == urlmap[i].active))
           {
   ...
   else
   {
       for (i = 0; i < URLMAP_SIZE; i++)
       {
           if (0 == memcmp(&urlmap[i].from.sin_addr, &ticket->from.sin_addr, sizeof(ticket->from.sin_addr))
               && (urlmap[i].from.sin_port == ticket->from.sin_port 
                    || htons(urlmap[i].from.sin_port) == sip_get_top_via_port(ticket))
               && 1 == urlmap[i].active)                  
           {
               /*后面判断type = RESTYP_INCOMING有的是根据response->from->url与urlmap表项比较，在forking的情况下会找错
               因此直接通过socket来判断index*/
       ...
       DEBUGC(DBCLASS_PROXY,"request is from LAN");
   }
   ...
   if ((ticket->index>=URLMAP_SIZE) || (ticket->index<0))
   {
      ERROR("proxy_request: ticket->index = %d", ticket->index);
      return STS_FAILURE;
   }
...
&&&*/
      ...
      if ((type == REQTYP_OUTGOING) && (0 != ticket->to.sin_addr.s_addr))//(type != REQTYP_INCOMING)
      {
         /* Request from SBC. */
   ...
   /* add my Via header line (outbound interface)*/
    ...
    if (type == REQTYP_OUTGOING) {
        sts = sip_rewrite_via(ticket, DIR_OUTGOING);
    }
    else
    {
        sts = sip_rewrite_via(ticket, DIR_INCOMING);
    }
#else
   if (type == REQTYP_INCOMING) {
   ...
   */
   ...
   sipsock_send(sendto_addr, port, ticket->protocol, buffer,
       buflen, is_t_online(request, i), type, ticket->tos, ticket->index); 
#else
   sipsock_send(sendto_addr, port, ticket->protocol, buffer, buflen, type, ticket->index);
#endif
   osip_free (buffer);
...
int proxy_response (sip_ticket_t *ticket) {
   ...
   else
   {
       for (i = 0; i < URLMAP_SIZE; i++)
       {
           if ((0 == memcmp(&ticket->from, &urlmap[i].from, sizeof(ticket->from))) && (1 == urlmap[i].active))
           {
               /*后面判断type = RESTYP_INCOMING有的是根据response->from->url与urlmap表项比较，在forking的情况下会找错
               因此直接通过socket来判断index*/
               index = i;
...
sts=sip_obscure_callid(ticket);
      ...
      if ((MSG_IS_RESPONSE_FOR(response,"INVITE")) ||
         ...
         /* 当响应是401或407鉴权时不关rtp流，防止401等重传关闭第一个通道后，invite再重传会新建不同的rtp端口 */
         } else if (((MSG_IS_STATUS_4XX(response)) && (response->status_code != 401 && \
...
#ifdef SUPPORT_ALG_PRIVILEGE_USER
   sipsock_send(sendto_addr, port, ticket->protocol, buffer,
         buflen, is_t_online(response, i), type, ticket->tos, ticket->index); 
#else
   sipsock_send(sendto_addr, port, ticket->protocol, buffer, buflen, type, ticket->index);
#endif
   osip_free (buffer);
...
int proxy_rewrite_request_uri(osip_message_t *mymsg, int idx){
   ...
   if (0 != memcmp(&urlmap[idx].from, &addr, sizeof(urlmap[idx].from)))
   {
      /* set the true host */
      if (url->host) {osip_free(url->host);url->host=NULL;}
      snprintf(chost, sizeof(chost), "%s", utils_inet_ntoa(urlmap[idx].from.sin_addr));
      DEBUGC(DBCLASS_BABBLE,"proxy_rewrite_request_uri: host=%s",
             chost);
      url->host = osip_strdup (chost);
      if (url->host == NULL) {
         ERROR("I'm trying to rewrite a request_uri but fail");
         return STS_FAILURE;
      }
   
      /* set the true port */
      if (url->port) {osip_free(url->port);url->port=NULL;}
      snprintf(cport, sizeof(cport), "%d", ntohs(urlmap[idx].from.sin_port));
      DEBUGC(DBCLASS_BABBLE,"proxy_rewrite_request_uri: port=%s",
             cport);
      url->port = osip_strdup (cport);
      if (url->port == NULL) {
         ERROR("I'm trying to rewrite a request_uri but fail");
         return STS_FAILURE;
      }
