...
static void *rtpproxy_main(void *arg) {
   ...
   for (;;) {
      ...
      ATP_UTIL_GetSysTime(&current_tv);
      ...
      if (rtp_proxytable[i].rtp_rx_sock &&
         ...
         if (k >= MAX_BRANCH_SIZE)
         {
             for (k = 0; k < MAX_BRANCH_SIZE; k++)
             {
                ...
                {
                    DEBUGC(DBCLASS_RTP,"rtp_proxytable[%d].remote_info[%d].wan_port:%d", i, k, rtp_proxytable[i].remote_info[k].wan_port);
...
int rtp_relay_stop_fwd (osip_call_id_t *callid,
                        int rtp_direction,
                        int media_stream_no, int nolock, int wan_port) {
   int i, k, sts;
   ...
   for (i=0; i<RTPPROXY_SIZE; i++) {
      ...
      if (rtp_proxytable[i].rtp_rx_sock &&
         ...
         if (wan_port != -1)
         {
             for (k = 0; k < MAX_BRANCH_SIZE; k++)
             {
                DEBUGC(DBCLASS_RTP,"stop rtp_proxytable[%d].remote_info[%d].wan_port:%d", i, k, rtp_proxytable[i].remote_info[k].wan_port);
             ...
             for (k = 0; k < MAX_BRANCH_SIZE; k++)
             {
                ...
                if (rtp_proxytable[i].remote_info[k].wan_port != 0)
                {
                    ...
                    //收到cancel的情况，需要考虑多个M行多个流的情况，不能直接退出，需要继续循环将其他的流的remote_info给清除掉
                    goto continue_loop;
                }
             }
         }
