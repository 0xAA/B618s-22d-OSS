...
#else
         struct sockaddr_in ticket_to;
         get_ip_by_ifname(configuration.inbound_if, &ticket_to.sin_addr);
         ticket_to.sin_port = configuration.sip_listen_port;
         
         sts = get_register_dst_ip(ticket.from.sin_addr, ntohs(ticket.from.sin_port), 
                                   ticket_to.sin_addr, ticket_to.sin_port,
                                   &ticket.to.sin_addr);
#endif
         DEBUGC(DBCLASS_PROXY, "ticket.to.sin_addr = %s, ntohs(ticket.from.sin_port) = %i.\r\n", utils_inet_ntoa(ticket.to.sin_addr), ntohs(ticket.from.sin_port));
      }
#endif
