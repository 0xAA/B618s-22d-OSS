...
#ifdef SUPPORT_ATP_CAPABILITY_SET
#ifdef SUPPORT_ATP_EUAP
#include "capability.h"
#include <linux/securebits.h>
#include <sys/prctl.h>
#define ATP_CAP_INFO_FILE   "/etc/atpcapinfo" /* 配置文件路径 */
#define TASK_COMM_LEN       16 /* 进程名长度 */
#define ATP_CAP_LINE_LEN    1024 /* 配置文件每行的长度 */
#define ATP_AID_SYSTEM  1000    /*Android下system权限的uid是1000*/
#define ATP_CAP_BIT(x)           (1ULL << (x))
#ifndef VOS_OK
#define VOS_OK (0)
#endif
#ifndef VOS_NOK
...
#ifdef SUPPORT_ATP_EUAP
/*!
  \brief 去除进程Capability能力集中req_caps之外的能力对应CapBnd标志位
      去除后，具有s标志位的bin文件最多可以继承该进程所拥有的Capability
  \param[in]  req_caps
  \retval void
*/
static void ATP_UTIL_DropBoundingCaps(VOS_UINT64 req_caps)
{
    VOS_UINT32 cap = 0;
    ...
    {
        VOS_UINT64 cap_mask = ATP_CAP_BIT(cap);
        ...
        if (prctl(PR_CAPBSET_READ, cap) > 0)
        {
            if (prctl(PR_CAPBSET_DROP, cap) < 0)
            {
                printf("failed to drop capability=%d, errno=[%d:%s]\n", cap, errno, strerror(errno));
            }
        }
    }
}
...
/*!
  \brief 设置进程Capability能力集，并将进程权限降为非root
  \param[in]  uid
  \param[in]  gid
  \param[in]  caps
  \retval VOS_OK 权限设置成功
*/
static VOS_UINT32 ATP_UTIL_ChangeProcessCaps(uid_t uid, gid_t gid, VOS_UINT64 caps)
{
    VOS_UINT32 cap = 0;
    VOS_UINT64 cap_mask = 0;
    ...
    /* SECBIT_KEEP_CAPS
              ...
              causes the thread to lose all capabilities.
       SECBIT_NO_SETUID_FIXUP
              Setting  this  flag  stops  the kernel from adjusting capability
              sets  when  the  thread's  effective  and  filesystem  UIDs  are
              switched  between  zero and nonzero values.                   */
    if(0 != prctl(PR_SET_SECUREBITS, SECBIT_KEEP_CAPS | SECBIT_NO_SETUID_FIXUP))
    {
        printf("could not set PR_SET_SECUREBITS on this process\n");
    }

    ATP_UTIL_DropBoundingCaps(caps);

    /* change gid */
    if (0 != setgid(gid))
    {
        printf("could not set gid\n");
        return VOS_NOK;
    }
    ...
    capheader.pid = 0; /* 0 is considered self pid */

    /* update the capabilities */
    for (cap = 0; cap <= CAP_LAST_CAP; cap++)
    {
        cap_mask = ATP_CAP_BIT(cap);
...
/*!
  \brief 获取进程的uid和gid等信息
  \param[in]  pcAppName: 进程名，预留给2C用于查询getpwnam，或后续2B增加进程group分组时查询映射表
  \param[out]  uid:
  \param[out]  gid:
  \return
  \retval 
*/
static VOS_UINT32 ATP_UTIL_GetProcessGroupInfo(const VOS_CHAR *pcAppName, uid_t *uid, gid_t *gid)
{
    if ((VOS_NULL == uid) || (VOS_NULL == gid))
    {
        return VOS_NOK;
    }
...
/*!
  \brief 获取进程的Capability能力集
  \param[in]  pcAppName: 进程名
  \param[out]  caps:
  \return
  \retval VOS_OK  获取成功
  \retval VOS_NOK 获取失败
*/
static VOS_UINT32 ATP_UTIL_GetProcessCapsInfo(const VOS_CHAR *pcAppName, VOS_UINT64 *caps)
{
    FILE*       pstFile = VOS_NULL;
    VOS_CHAR    acAppName[TASK_COMM_LEN] = {0};
    VOS_CHAR    acLine[ATP_CAP_LINE_LEN] = {0};
    VOS_UINT64  ullCaps = 0;
    VOS_INT32   nNum = 0;
    VOS_UINT32  ulRet = VOS_NOK;
    
    if ((VOS_NULL == pcAppName) || (VOS_NULL == caps))
    {
        return ulRet;
    }
    ...
    *caps = 0;
    ...
    pstFile = fopen(ATP_CAP_INFO_FILE, "r");
    if (VOS_NULL != pstFile)
    {
        while (fgets(acLine, ATP_CAP_LINE_LEN, pstFile))
        {
            // 空行
            if (1 >= strlen(acLine))
            {
            ...
            // 此处sscanf_s根据传入参数，返回结果应为2
            nNum = sscanf(acLine, "%s = %llu", acAppName, &ullCaps);
            if (2 == nNum)
            {
                if (0 == strncmp(acAppName, pcAppName, TASK_COMM_LEN))
                {
                    *caps = ullCaps;
                    ulRet = VOS_OK;
                    break;
...
*/
static VOS_UINT32 ATP_UTIL_ForceSetPrivileges(const VOS_CHAR *pcAppName)
{
#ifdef SUPPORT_ATP_EUAP
    VOS_UINT64 ullCaps = 0;
    uid_t uid = 0;
    ...
    // 此处不关注返回值，获取Capability能力集失败，caps为0就只做降权处理
    (void)ATP_UTIL_GetProcessCapsInfo(pcAppName, &ullCaps);

    ulRet = ATP_UTIL_GetProcessGroupInfo(pcAppName, &uid, &gid);
    if (VOS_OK != ulRet)
    {
        return VOS_NOK;
    }

    ulRet = ATP_UTIL_ChangeProcessCaps(uid, gid, ullCaps);
...
#ifdef SUPPORT_ATP_CAPABILITY_SET
    ATP_UTIL_ForceSetPrivileges("cupsd");
#endif
