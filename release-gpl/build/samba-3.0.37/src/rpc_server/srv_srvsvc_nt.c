...
/*******************************************************************
 True if service name is user name.
 ********************************************************************/
static BOOL is_user_same_service(pipes_struct *p, int snum)
{
//2B need this ,2C no need
#if defined(SUPPORT_ATP_EUAP) && defined(SUPPORT_ATP_SAMBA_USER_SMBUSERS)
    const char *net_name = lp_servicename(snum);
    user_struct *user = get_valid_user_struct(p->vuid);
    
    if (!user || !net_name) {
        /* Null user, we return all services */
        return True;
    }
    
    DEBUG(0,("Check service = [%s], brownsable = %d, printer = [%s], Printok = [%d]\n", 
        net_name, 
        lp_browseable(snum), 
        lp_printername(snum), 
        lp_print_ok(snum)));
    ...
    if (strwicmp(net_name, "IPC") == 0 || strwicmp(net_name, "IPC$") == 0){
    ...
    if (strwicmp(net_name, user->user.smb_name) == 0 || strwicmp(net_name, user->user.unix_name) == 0){
        return True;
    }
	       ...
	       uint32 info_level, uint32 *resume_hnd, uint32 *total_entries, BOOL all_shares)
{
	...
	*resume_hnd = 0;
	...
	for (snum = 0; snum < num_services; snum++) {
		if (is_user_same_service(p, snum) && lp_browseable(snum) && lp_snum_ok(snum) && (all_shares || !is_hidden_share(snum)) )
			num_entries++;
	}
