...
#endif
  {"IP_FROMLAN",           IPPROTO_IP,      IP_FROMLAN,       1,           OPT_INT},
  {"IPV6_FROMLAN",         IPPROTO_IPV6,    IPV6_FROMLAN,     1,           OPT_INT},
		...
		bool rebind)
{
	...
	/* This block sets/clears the SO_REUSEADDR and possibly SO_REUSEPORT. */
	{
		...
		if( setsockopt(res,SOL_SOCKET,SO_REUSEPORT,
    	    ...
    	    if (setsockopt(res, SOL_SOCKET, SO_BINDTODEVICE, "br0", strlen("br0") + 1) < 0)
    	    {
    	        printf("bind SO_BINDTODEVICE error. %d\n", errno);
    	    }
