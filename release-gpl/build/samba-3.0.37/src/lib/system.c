...
int smb_count;
struct passwd rootpw[SMB_MAX_USER_NUM + 2];
char *rootmem[SMB_MAX_USER_NUM + 2];
char rootmem_buf[SMB_MAX_USER_NUM + 2][SMB_MAX_USER_MEM_LEN];
...
#endif
#ifdef SUPPORT_ATP_SAMBA_USER_SMBUSERS
#ifdef SUPPORT_ATP_ATP_HTML_CHANNEL
    sbuf->st_gid = 0;
#endif
#endif
...
struct passwd *sys_getpwnam(const char *name)
{
    ...
    {
        DEBUG(5,("name=[%s] ++ rootpw[%d].pw_name=[%s]\n", name, i, rootpw[i].pw_name));
        if(0 ==strcmp(name, rootpw[i].pw_name))
        {
            DEBUG(5,("find!!!!!!!!!!!!!!!!!!!!!!! \n"));
            return &rootpw[i];
        }
...
struct passwd *sys_getpwuid(uid_t uid)
{
    ...
    {
        DEBUG(5,("uid = %d\n", uid));
        if(uid == rootpw[i].pw_uid)
        {
            DEBUG(5,("uid=rootpw[i].pw_uid\n"));
            return &rootpw[i];
        }
    }
