...
#define STRUCT_PASSWD_PARA_NEED_MEM 5
#define SMB_FILE_LINE_MAX_LEN 256
#define STRUCT_PASSWD_PARA_MAX_MEM 512
#define SAMBA_USER_FILE  "/var/samba/smbusers"
extern struct passwd rootpw[SMB_MAX_USER_NUM + 2];
extern char *rootmem[SMB_MAX_USER_NUM + 2];
extern  char rootmem_buf[SMB_MAX_USER_NUM + 2][SMB_MAX_USER_MEM_LEN];
static char rootpw_buf[SMB_MAX_USER_NUM + 2][STRUCT_PASSWD_PARA_NEED_MEM][STRUCT_PASSWD_PARA_MAX_MEM];
extern int smb_count;
#endif
...
int SambaUserInfoInit(void)
{
    ...
    memset(rootmem, 0, sizeof(rootmem));
    memset(rootmem_buf, 0, sizeof(rootmem_buf));
    
    pSetPwdd = fopen(SAMBA_USER_FILE, "r");
    if (NULL == pSetPwdd)
    {
        DEBUG(0,("fopen SAMBA_USER_FILE error\n"));
        return 0;
    }
    if(NULL != pSetPwdd)
    {
        smb_count = 0;
        for ( i = 0; i < SMB_MAX_USER_NUM + 2; i++)
        {
            if(NULL == fgets(tmpbuf, SMB_FILE_LINE_MAX_LEN, pSetPwdd))
            {
                DEBUG(0,("fgets tmpbuf is null error\n"));
                break;
            }
            
            p = &rootpw[i];
            p->pw_name = &rootpw_buf[i][0][0];
            p->pw_passwd = &rootpw_buf[i][1][0];
            pw_gecos = &rootpw_buf[i][2][0];
            p->pw_dir = &rootpw_buf[i][3][0];
            p->pw_shell = &rootpw_buf[i][4][0];

            DEBUG(0,("tmpbuf %s\n", tmpbuf));
            sscanf(tmpbuf, "%s %s %lu %lu %s %s %s\n",
                    p->pw_name, p->pw_passwd,
                    (unsigned long*)&(p->pw_uid),
                    (unsigned long*)&(p->pw_gid),
                    pw_gecos, p->pw_dir, p->pw_shell);
            smb_count++;
        }

        for ( i= 0; i < smb_count; i++)
        {
            rootmem[i] = &rootmem_buf[i][0];
            snprintf(rootmem[i], SMB_MAX_USER_MEM_LEN, "%s", p->pw_name); 
        }
    ...
    for (i = 0; i < smb_count; i++)
    {
        DEBUG(0,("(%s)(%s)(%s)(%s)(%d)\n", rootpw[i].pw_name,rootpw[i].pw_dir,
                 rootpw[i].pw_passwd, rootpw[i].pw_shell, rootpw[i].pw_uid));
    }
...
*/
    ...
    ATP_LoginCtlInit(ATP_LOGINTYPE_SAMBA);
#endif
#ifdef HAVE_SET_AUTH_PARAMETERS
	set_auth_parameters(argc,argv);
#endif
#ifdef NOT_HAVE_POPT
	while ((ch = getopt(argc, argv, "DV")) != EOF) {
		switch(ch) {
		case 'D':
			is_daemon = true;
			break;
