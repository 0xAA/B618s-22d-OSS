...
int reply_mkdir(connection_struct *conn, char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	//Need to exclude tmpfs, romfs and ramfs.
	memset( &statfsbuf, 0, sizeof( statfsbuf ) );

	if (statfs(conn->connectpath, &statfsbuf) >= 0)
	{
		//not tmpfs, romfs and ramfs
	    if (((statfsbuf.f_type) != ATP_USB_MOUNT_TMPFS_MAGIC) &&
		  ((statfsbuf.f_type) != ATP_USB_MOUNT_ROMFS_MAGIC) &&
		  ((statfsbuf.f_type) != ATP_USB_MOUNT_RAMFS_MAGIC))
	    {
	        DEBUG(5, ("check file system type (%s):(%lx)\n", conn->connectpath, statfsbuf.f_type ));
	    }
	    else
	    {
	        DEBUG(5, ("Can't write outside the disk dirctory (%s):(%lx)\n", conn->connectpath, statfsbuf.f_type ));
	        END_PROFILE(SMBmkdir);
