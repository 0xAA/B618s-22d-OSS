...
NTSTATUS reduce_name(connection_struct *conn, const pstring fname)
{
	...
	DEBUG(3,("reduce_name realpath [%s] -> [%s], conn->connectpath [%s], print_ok = [%d]\n", fname, resolved_name, conn->connectpath, lp_print_ok(SNUM(conn))));
    ...
    if (strncmp(resolved_name, "/mnt", strlen("/mnt")) && !lp_print_ok(SNUM(conn)))
    {
		if (free_resolved_name) {
			SAFE_FREE(resolved_name);
		}
        DEBUG(0,("reduce_name: realpath doesn't match mnt dir !\n"));
        return NT_STATUS_OBJECT_NAME_INVALID;
    }

    
	/* Check for widelinks allowed. */
	/*  [PATCH] CVE-2015-5252: s3: smbd: Fix symlink verification  */
