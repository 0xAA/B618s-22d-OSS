...
#include "includes.h"
#include "atplog.h"
#include "gpldevstatusutil.h"
#ifdef SUPPORT_ATP_SAMBA_LOGIN_CONTROL  //added by qichenghong 2009.12.03
#include "loginctl.h"
#endif
#define TR140_SAFE_AYNOYMOUS_USERNAME    "******"
...
#endif
        {
            ATP_LOG_LogItemPrint(ATP_SYS_LOG_UMOUNT_SAMBA_LONGIN_SUCCESS,
                                 "guest",
                                 client_addr());
...
static int login_control(int success, const char* username)
{
#ifdef SUPPORT_ATP_SAMBA_LOGIN_CONTROL 
    VOS_UINT32 ulLogType = 0;
    VOS_UINT ulLogStatus = 0;
    ATP_SCTRL_LOGIN_RET_EN enLoginRet = ATP_SCTRL_LOGIN_RET_SUCCESS;

    ulLogType = ATP_LOGINTYPE_SAMBA;

    if (success)
    {
        ulLogStatus = LOGIN_OK;
    }
    else
    {
        ulLogStatus = LOGIN_FAIL;
    }
    DEBUG(3,("Check Log status [%d]\n", ulLogStatus));
    enLoginRet = ATP_LoginCtlCheck(username , client_addr() , ulLogType, ulLogStatus);
    if (ATP_SCTRL_LOGIN_RET_SUCCESS == enLoginRet)
    {
        return 0;
    }
    else if(ATP_SCTRL_LOGIN_RET_CONTROL == enLoginRet)
    {
        return -2;
    }
    else
			     ...
			     AUTH_NTLMSSP_STATE **auth_ntlmssp_state)
{
    ...
    /* 
    * 安全问题,需要添加登录SAMBA/FTP的日志,包括失败的.这里的log和DT产品不一致
    * 由于ATP产品的samba暂时没有开SUPPORT_LOGIN_CONTROL -> SUPPORT_ATP_SAMBA_LOGIN_CONTROL,而724开了
    * 暂时把日志放在宏的外面,后续整改Login Control功能的时候,根据需求注意统一.
    */  
    if (((*auth_ntlmssp_state)->ntlmssp_state)
        && ((*auth_ntlmssp_state)->ntlmssp_state->user)
        && (strlen((*auth_ntlmssp_state)->ntlmssp_state->user)))
    {
        if(NT_STATUS_IS_OK(status))
        {
            DEBUG(3,("ATP: Check Username [%s][%s][%s] OK\n", client_addr(),
                (*auth_ntlmssp_state)->ntlmssp_state->domain,
                (*auth_ntlmssp_state)->ntlmssp_state->user));
            if (0 != login_control(1, (*auth_ntlmssp_state)->ntlmssp_state->user))
            {
                DEBUG(0,("ATP: Username [%s][%s][%s] FORBIDDEN\n", client_addr(),
                    (*auth_ntlmssp_state)->ntlmssp_state->domain,
                    (*auth_ntlmssp_state)->ntlmssp_state->user));
            ...
            else
            {
                ...
                ATP_LOG_LogItemPrint(ATP_SYS_LOG_UMOUNT_SAMBA_LONGIN_SUCCESS, (*auth_ntlmssp_state)->ntlmssp_state->user, client_addr());
#else
                ATP_LOG_LogItemPrint(ATP_SYS_LOG_UMOUNT_SAMBA_LONGIN_SUCCESS, TR140_SAFE_AYNOYMOUS_USERNAME, client_addr());
#endif
                ATP_DevStatus_UsedCountAdd("samba");
            }
        }
        else
        {
            DEBUG(0,("ATP: Check Username [%s][%s][%s] FAILED\n", client_addr(),
                (*auth_ntlmssp_state)->ntlmssp_state->domain,
                (*auth_ntlmssp_state)->ntlmssp_state->user));
            if( -2 != login_control(0, (*auth_ntlmssp_state)->ntlmssp_state->user) )
            {
#ifdef SUPPORT_ATP_EUAP
                ATP_LOG_LogItemPrint(ATP_SYS_LOG_UMOUNT_SAMBA_LONGIN_FAILED, (*auth_ntlmssp_state)->ntlmssp_state->user, client_addr());
#else
                ATP_LOG_LogItemPrint(ATP_SYS_LOG_UMOUNT_SAMBA_LONGIN_FAILED, TR140_SAFE_AYNOYMOUS_USERNAME, client_addr());
#endif
            }
        }
    }
					...
					int length,int bufsize)
{
	...
	/* 
	 * 解决peach攻击时导致samba挂死问题
	 * 从log可以发现,攻击时报文异常,提示check_spnego_blob_complete: zero blob length !
	 * 此时blob1没有赋值,而check_spnego_blob_complete返回OK,导致这里的调用blob1.data是空指针,因此加上
	 */
			  ...
			  int length,int bufsize)
{
    ...
    if (!*user) {
        ATP_LOG_LogItemPrint(ATP_SYS_LOG_UMOUNT_SAMBA_LONGIN_SUCCESS, "anonymous", client_addr());
        ATP_DevStatus_UsedCountAdd("samba");
    }
