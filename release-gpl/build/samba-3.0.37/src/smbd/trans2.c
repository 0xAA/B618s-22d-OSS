...
cBytesSector=%u, cUnitTotal=%u, cUnitAvail=%d\n", (unsigned int)bsize, (unsigned int)sectors_per_unit,
		...
		case SMB_QUERY_POSIX_FS_INFO:
		{
			...
			rc = SMB_VFS_STATVFS(conn, conn->dirpath, &svfs);
	...
	*ppdata = (char *)SMB_REALLOC(*ppdata, data_size); 
	...
	switch (info_level) {
		...
		case SMB_QUERY_FILE_UNIX_BASIC:
			/*xiaomi player box need other group to read and excute rights for the current dir (.) configured in config file*/
			if (0 == strcmp(fname, "."))
			{
				sbuf.st_mode |= 0x5;
				DEBUG(0,("call_trans2qfilepathinfo: SMB_QUERY_FILE_UNIX_BASIC set other group to read and excute mode is [%o]\n", sbuf.st_mode));
			}
				...
				SMB_STRUCT_STAT *psbuf)
{
    ...
    status = refuse_symlink(conn, fsp->fsp_name);
    if (!NT_STATUS_IS_OK(status)) {       
        return status; 
    }
