...
struct LAC_Config g_stLacConfig;
extern struct sockaddr_in server;

struct PPPC_Config g_stPppcConfig;
char g_acL2tpAuthConfig[PPPC_MAX_LEN];
...
{
    FILE *fp =NULL;
    char path[PPPC_PATH_LAN] = {0};
    l2tp_log(LOG_ERR, "into writel2tpconf!\n");
    snprintf(path, sizeof(path), "%s", L2TP_SECRETS_FILE);

    fp = fopen(path, "w");
    if (NULL == fp)
    {
        l2tp_log(LOG_ERR, "Read pppc config fail，please check!!!\n");
        return;
    }
...
static void readl2tpconf()
{
    FILE *fp =NULL;
    char path[PPPC_PATH_LAN] = {0};
    char cmd[PPPC_MAX_LEN] = {0};

    l2tp_log(LOG_ERR, "into readl2tpconf!\n");
    
    memset(g_acL2tpAuthConfig,0,sizeof(g_acL2tpAuthConfig));
    
    snprintf(path, sizeof(path), "%s", L2TP_SECRETS_FILE);
    fp = fopen(path, "r");
    if (NULL == fp)
    {
        l2tp_log (LOG_DEBUG, "%s: Read l2tp-secrets config fail，please check!!!\n", __FUNCTION__);
        return;
    }
    
    /*读取/var/l2tp/l2tp-secrets文件*/ 
    while(fgets(cmd, PPPC_MAX_LEN,fp)!=NULL)
    {
        if(cmd[strlen(cmd)-1] == '\n')
        {
        ...
        l2tp_log(LOG_ERR, "into readl2tpconf read l2tp-secrets!\n");
        
        if(NULL != strstr(cmd,"*"))
        {            
            snprintf(g_acL2tpAuthConfig, sizeof(g_acL2tpAuthConfig), "%s", cmd);
        }
    }
    fclose(fp);

    l2tp_log(LOG_ERR, "into readl2tpconf is ok!\n");
    /**< 清除/var/l2tp/l2tp-secrets文件 */
    unlink(path);    
}

static void readpppconf()
{
    FILE *fp =NULL;
    char path[PPPC_PATH_LAN] = {0};
    char cmd[PPPC_MAX_LEN] = {0};
    
    l2tp_log(LOG_ERR, "into readpppconf!\n");
    memset(&g_stPppcConfig,0,sizeof(struct PPPC_Config));
    snprintf(path, sizeof(path), L2TP_PPP_CONFIG_FILE, L2TP_PPP_IFC_NAME);

    fp = fopen(path, "r");
    if (NULL == fp)
    {
        l2tp_log (LOG_DEBUG, "%s: Read pppc config fail，please check!!!\n", __FUNCTION__);
        return;
    }
    ...
    while(fgets(cmd, PPPC_MAX_LEN,fp)!=NULL)
    {
        ...
        if(NULL != strstr(cmd,"username = "))
        {
            snprintf(g_stPppcConfig.acUserName, sizeof(g_stPppcConfig.acUserName), "%s", cmd);
        }
        else if(NULL != strstr(cmd,"password = "))
        {
            snprintf(g_stPppcConfig.acPassWord, sizeof(g_stPppcConfig.acPassWord), "%s", cmd);
        }
        else if(NULL != strstr(cmd,"protocol = "))
        {
            snprintf(g_stPppcConfig.acProtocol, sizeof(g_stPppcConfig.acProtocol), "%s", cmd);
        }
        else if (NULL != strstr(cmd, "mru = "))
        {
            snprintf(g_stPppcConfig.acMru, sizeof(g_stPppcConfig.acMru), "%s", cmd);
        }
        else if (NULL != strstr(cmd, "mtu = "))
        {
            snprintf(g_stPppcConfig.acMtu, sizeof(g_stPppcConfig.acMtu), "%s", cmd);
        }
        else if (NULL != strstr(cmd, "auth = "))
        {
            snprintf(g_stPppcConfig.acAuth, sizeof(g_stPppcConfig.acAuth), "%s", cmd);
        }
    }

    fclose(fp);

    /** 删除L2TP_PPP_CONFIG_FILE */
    unlink(path);

    /** 读/var/l2tp/l2tp-secrets */
    readl2tpconf();
    
}

static void writepppconf()
{
    FILE *fp =NULL;
    char path[PPPC_PATH_LAN] = {0};
    ...
    //此项在pppc使用pppol2tp插件时无意义,只给pppc解析用
    fprintf(fp, "nasname = ttyUSB8\n");


    //配置网关自己拨号用的用户名和密码
    fprintf(fp, "%s\n", g_stPppcConfig.acUserName);
    fprintf(fp, "%s\n", g_stPppcConfig.acPassWord);

    fprintf(fp, "proxy = 0\n");
    fprintf(fp, "lanmax = 4\n");
    fprintf(fp, "bind = 0\n");

    //protocol 为5,即让pppc使用pppol2tp插件
    //pppcapi.h  ATP_PPPC_PROTOCOL_EN
    fprintf(fp, "%s\n",  g_stPppcConfig.acProtocol);
    ...
    fprintf(fp, "mbs = 0\n");
    fprintf(fp, "scr = 0\n");
    fprintf(fp, "%s\n",  g_stPppcConfig.acAuth);
    fprintf(fp, "idletime = 0\n");
    fprintf(fp, "dialnumber = \n");
    fprintf(fp, "setapn = \n");
    fprintf(fp, "session = \n");
    fprintf(fp, "servicename = \n");
    fprintf(fp, "acname = \n");
    fprintf(fp, "ulRandomDial = 0\n");
    fprintf(fp, "ulRetryDial = 0\n");
    fprintf(fp, "ulRetrySend = 0\n");
    fprintf(fp, "ulKeepAlive = 0\n");
    fprintf(fp, "dial3times = \n");
    fprintf(fp, "%s\n", g_stPppcConfig.acMtu);
    fprintf(fp, "%s\n", g_stPppcConfig.acMru);
    fprintf(fp, "IPv4Mode = 1\n");
    fprintf(fp, "IPv6Mode = 0\n");
    fprintf(fp, "IPv6ManagedAddress = 0\n");
    fprintf(fp, "ReqTime = 10\n");
    fprintf(fp, "pvcsearchtime = 0\n");
    fprintf(fp, "servicelist = OTHER\n");
...
int start_pppd (struct call *c, struct ppp_opts *opts)
{
    ...
    l2tp_log(LOG_INFO, "%s: kernel_support=%d\n", __FUNCTION__, kernel_support);
    ...
    if (kernel_support)
    {
       ...
       l2tp_log(LOG_INFO, "CCCCCCCC  peer addr=%s\n", inet_ntoa(c->container->peer.sin_addr));
       l2tp_log(LOG_INFO, "CCCCCCCC  peer port=%d\n", ntohs(c->container->peer.sin_port));

       l2tp_log(LOG_INFO, "CCCCCCCC  server_socket=%d\n", server_socket);
       l2tp_log(LOG_INFO, "CCCCCCCC  sax.pppol2tp.pid=%d\n", sax.pppol2tp.pid);
       l2tp_log(LOG_INFO, "CCCCCCCC  sax.pppol2tp.s_tunnel=%d\n", sax.pppol2tp.s_tunnel);
       l2tp_log(LOG_INFO, "CCCCCCCC  sax.pppol2tp.s_session=%d\n", sax.pppol2tp.s_session);
       l2tp_log(LOG_INFO, "CCCCCCCC  sax.pppol2tp.d_tunnel=%d\n", sax.pppol2tp.d_tunnel);
       l2tp_log(LOG_INFO, "CCCCCCCC  sax.pppol2tp.d_session=%d\n", sax.pppol2tp.d_session);
       int ret = 0;
       errno = 0;
        for (;;)
        {
            ret = connect(fd2, (struct sockaddr *)&sax, sizeof(sax) );
            if (ret < 0 ) 
            {
                ...
                l2tp_log (LOG_ERR, "XXXX connect to PPPoL2TP session socket failed, errno=%d(%s)\n", errno,strerror(errno));
            }
            break;
        }
        if (ret < 0)
        {
            l2tp_log (LOG_WARNING, "XXXX pppol2tp session socket failed!\n");
            return -errno;
            
        }
        else
        {
            //write(fd2, "this is a packet.", 20);
            l2tp_log (LOG_WARNING, "XXXX pppol2tp session socket connect OK, fd2=%d\n", fd2);
        }
#if 0
...
#endif
       stropt[pos++] = strdup ("-I");
       stropt[pos++] = strdup (L2TP_PPP_IFC_NAME);
	...
	close(fd2);
     ...
     //使用平台pppc组件拨号时的控制文件
        if (LAC_DIALLING == g_stLacConfig.dialling_mode)
        {
            sprintf(acPath, L2TP_PPP_DIAL_FILE, L2TP_PPP_IFC_NAME);
            lfd = open(acPath, O_CREAT | O_WRONLY | O_TRUNC, 0755);
            if (lfd >= 0)
            {
                //pppc一起动,就需要与LNS侧的PPP服务器拨号
                write(lfd, "3", 1);
                close(lfd);
            }
        }
        ...
        /**< 启动pppc前，写L2TP_PPP_CONFIG_FILE */
        writepppconf();

#ifdef __BIONIC__
        execv("/system/bin/pppc", stropt);
#else
        execv("/bin/pppc", stropt);
#endif
...
void magic_lac_dial (void *data)
{
    struct lac *lac;
    lac = (struct lac *) data;
    static unsigned int ultunelmax = 0;
    
    l2tp_log (LOG_INFO, "into magic_lac_dial: ultunelmax = %d.\n", ultunelmax);    
    if (!lac)
    {
    ...
    lac->rsched = NULL;
    l2tp_log (LOG_INFO, "%s: rmax:%d, retries:%d.\n", __FUNCTION__, lac->rmax, lac->rtries);
    ...
    {
        ultunelmax++;
        l2tp_log (LOG_INFO, "%s: ultunelmax = %d.\n", __FUNCTION__, ultunelmax);
        if (ultunelmax > lac->rmax)
        {
            l2tp_log (LOG_INFO, "%s: ultunelmax exceeded.\n", __FUNCTION__);
            ultunelmax = 0;
            return;
        } 
...
{
    control_fd = open (gconfig.controlfile, O_RDONLY | O_NONBLOCK | O_CREAT, 0600);
    if (control_fd < 0)
    {
...
void init (int argc,char *argv[])
{
    ...
    rand_source = 0;
    init_addr ();
    memset(&g_stLacConfig,0,sizeof(struct LAC_Config));
    if (init_config ())
    {
...
int main (int argc, char *argv[])
{
    ...
    /** 删除L2TP_CONFIG_FILE */
    unlink(gconfig.configfile);

    /**< 读取L2TP_PPP_CONFIG_FILE文件，保存其内容*/
    readpppconf();
