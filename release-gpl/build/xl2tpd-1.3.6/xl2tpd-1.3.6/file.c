...
char filerr[STRLEN];
/* add by dongligang 2008-07-28 begin */

int set_lns (char *word, char *value, int context, void *item);
/* add by dongligang 2008-07-28 end */
...
int init_config ()
{
    ...
    set_lns("lac",inet_ntoa(g_stLacConfig.lns_address),CONTEXT_LAC,laclist);
    l2tp_log (LOG_WARNING, "%s: this lns addr %s\n",
                 __FUNCTION__, inet_ntoa(g_stLacConfig.lns_address));
    if (g_stLacConfig.connect_type == CON_MANUAL)
    {
         laclist->redial = 0;
    }
...
struct lac *new_lac ()
{
    ...
    tmp->chap_refuse = 0;
    tmp->mschap_refuse = 0;
    tmp->mschap_require = 0;
    tmp->mschapv2_refuse = 0;
    tmp->mschapv2_require = 0;
    tmp->mschap_encryalg = ENCRYPTION_NULL;
    tmp->t = NULL;
    tmp->redial = 0;
    tmp->rtries = 0;
    tmp->rmax = 0;
...
int set_hellotime(char *word, char *value, int context, void *item)
{
    if (atoi (value) < 1)
    {
        snprintf (filerr, sizeof (filerr),
    ...
    {
    case CONTEXT_LAC:
        set_int (word, value, &(g_stLacConfig.tunnel_keep_alive_time));
        l2tp_log(LOG_INFO, "set hello time=%d\n", g_stLacConfig.tunnel_keep_alive_time);
        break;
    default:
        snprintf (filerr, sizeof (filerr), "'%s' not valid in this context\n",
                  word);
...
int set_dialmode(char *word, char *value, int context, void *item)
{
    switch (context & ~CONTEXT_DEFAULT)
    {
    case CONTEXT_LAC:
        if (set_boolean (word, value, &(g_stLacConfig.dialling_mode)))
            return -1;
        break;
    default:
        snprintf (filerr, sizeof (filerr), "'%s' not valid in this context\n",
    ...
    l2tp_log(LOG_INFO, "set pc dialing mode=%d\n", g_stLacConfig.dialling_mode);
    return 0;
}
...
struct keyword words[] = {
    ...
    {"rx bps", &set_speed},
    {"bps", &set_speed},
	{"hello time", &set_hellotime},
    /*pc dialing = yes, 则为PC先pppoe拨号到网关，网关代理；为no,则为网关自己ppp拨号*/
    {"pc dialing", &set_dialmode},
    {NULL, NULL}
};
