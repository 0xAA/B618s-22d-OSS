...
/*	if (c->dethrottle) deschedule(c->dethrottle); */
    ...
    if (pid)
    {
       ...
       */
        ...
        if (LAC_DIALLING == g_stLacConfig.dialling_mode)
        {
            ...
            sprintf(acPath, L2TP_PPP_STATUS_FILE, L2TP_PPP_IFC_NAME);
            lfd = open(acPath, O_CREAT | O_WRONLY | O_TRUNC, 0755);
            if (lfd >= 0)
            {
                //disconnected status, pppcapi.h
                write(lfd, "6", 1);
                close(lfd);
            }
        }
    ...
    if (c->container)
    {
        ...
        l2tp_log(LOG_DEBUG, "There are %d calls left.\n", c->container->count);

        if (c->container->count == 0)
        {
            l2tp_log(LOG_DEBUG, "go to call call_close()\n");
            call_close(c->container->self);
        }
    }
    if (c->lac)
    {
