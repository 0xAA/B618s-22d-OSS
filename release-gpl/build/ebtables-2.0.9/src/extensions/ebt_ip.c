...
#define IP_DRANGE 'a'
#endif
//?a??biolib ?D?T﹞“那1車?o‘那ygetprotobyname ?那足a
struct ebtprot {
	char *name;
	u_int8_t num;
};
static const struct ebtprot ebt_chain_protos[] = {
	{ "tcp", IPPROTO_TCP },
	{ "udp", IPPROTO_UDP },
};
...
parse_iprange(char *arg, struct ebt_iprange *range)
{
	...
	if (undot_ip(arg, (unsigned char *)&ip))
		ebt_print_error("iprange match: Bad IP address `%s'\n", 
			   arg);
	range->min_ip = ip;
   ...
   unsigned int *flags, struct ebt_entry_match **match)
{
	...
	case IP_PROTO:
		...
		if (*end != '\0') {
			struct protoent *pe;
			if (pe = getprotobyname(optarg))
				ipinfo->protocol = pe->p_proto;
			else {
			unsigned int i;
			for (i = 0;
			     i < sizeof(ebt_chain_protos)/sizeof(struct ebtprot);
			     i++) {
				if (strcmp(optarg, ebt_chain_protos[i].name) == 0) {
					ipinfo->protocol = ebt_chain_protos[i].num;
					break;
				}
			}
			if (i == sizeof(ebt_chain_protos)/sizeof(struct ebtprot))
				ebt_print_error2("Unknown specified IP protocol - %s",
				     argv[optind - 1]);
			}
		} else {
