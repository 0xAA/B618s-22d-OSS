        ...
        char *phonenr,int window)
{
    ...
    if (NULL == conn || NULL == conn->call)
    {
        fatal("conn or conn->call is NULL\n");
    }
...
void pptp_call_close(PPTP_CONN * conn, PPTP_CALL * call)
{
    ...
    if (NULL == conn || NULL == conn->call || NULL == call)
    {
        fatal("conn or conn->call is NULL\n");
    }
...
void pptp_call_destroy(PPTP_CONN *conn, PPTP_CALL *call)
{
    if (NULL == conn || NULL == conn->call || NULL == call)
    {
        fatal("NULL at pptp_call_destroy\n");
    }
...
void pptp_conn_close(PPTP_CONN * conn, u_int8_t close_reason)
{
    ...
    /*pptp_send_ctrl_packet可能会destroy conn*/
    if (conn)
    {
                 ...
                 int * max_fd)
{
    ...
    if (FD_SETSIZE < conn->inet_sock)
    {
        fatal("conn->inet_sock [%d] is invalid\n", conn->inet_sock);
    }
...
int pptp_dispatch(PPTP_CONN * conn, fd_set * read_set, fd_set * write_set)
{
    ...
    /*前面的pptp_handle_timer可能会释放conn*/
    if (NULL == global.conn || NULL == global.conn->call) {
        exit(0);
    }

    if (FD_SETSIZE < conn->inet_sock) {
        /*exit this session to trigger restart*/
        exit(0);
    }
...
int pptp_make_packet(PPTP_CONN * conn, void **buf, size_t *size)
{
    ...
    if (NULL == conn || NULL == conn->call || NULL == buf || NULL == size)
    {
        fatal("has NULL para\n");
    }
...
int pptp_send_ctrl_packet(PPTP_CONN * conn, void * buffer, size_t size)
{
    if (NULL == conn || NULL == conn->call || NULL == buffer)
    {
        fatal("conn or conn->call or buffer is NULL\n");
    }
...
static void pptp_handle_timer()
{
    ...
    if (global.conn->conn_state != CONN_ESTABLISHED) {
        if (global.conn->conn_state == CONN_WAIT_STOP_REPLY)
        {
            ...
            /*destroy后，global.conn内存已经释放了，不能继续走后续流程*/
            fatal("conn destroyed, trigger next\n");
        }
        else /* soft close */
	...
	pptp_conn_close(global.conn, PPTP_STOP_NONE);
        ...
        /*pptp_send_ctrl_packet 可能释放global.conn内存*/
        if (NULL == global.conn)
        {
            fatal("global.conn is destroyed\n");
        }
