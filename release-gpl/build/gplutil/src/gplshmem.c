...
#ifdef __BIONIC__
#define GET_IPC_PERM_KEY(perm)		((perm)->key)
#else
#define GET_IPC_PERM_KEY(perm)		((perm)->__key)
#endif
...
ATP_STATIC VOS_BOOL gvarShmExist(VOS_UINT32 ulModIdx)
{
    ...
    for (i = 0; i < lShmNum + 1; i++)   // !! lShmNum + 1
    {
	    ...
	    if (lKey == (VOS_INT32)GET_IPC_PERM_KEY(ipcp))    // __key??
	    //(lKey == (VOS_INT32)ipcp->__key)    // __key??
        {
            return VOS_TRUE;
        }
    }
...
VOS_VOID* ATP_UTIL_BgetStrdup(VOS_VOID* pvBgetHandle, const VOS_VOID* pvString)
{
    ...
    ulLen = strlen(pvString) + 1;
    pszTmp = ATP_UTIL_BgetMalloc(pvBgetHandle, ulLen);
    if (pszTmp)
    {
        snprintf(pszTmp, ulLen, "%s", (const VOS_CHAR *)pvString);
        return pszTmp;
    }
    else
    {
