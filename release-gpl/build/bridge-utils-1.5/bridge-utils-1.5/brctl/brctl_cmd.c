...
static int br_cmd_add_portrelay(int argc, char*const* argv)
{
    int iRet;
    int iEnable;
    char devname[64] = {0};
    ...
    memset(devname, 0, 64);	
	snprintf(devname, sizeof(devname), "%s", argv[index]);

    index++;
    if(sscanf(argv[index], "%i", &iEnable) != 1)
    {
        fprintf(stderr, "bad value\n");
        return 1;
    }

    iRet= br_set_portrelay(devname,iEnable);

    if (iRet)
    {
        fprintf(stderr, "enable iptv failed: %s\n",
    	strerror(errno));
    }
...
static const struct command commands[] = {
    ...
    {2, "setportrelay",br_cmd_add_portrelay,
       "<WanInterface> <Relay>\t InterfaceName Enable" },
};
