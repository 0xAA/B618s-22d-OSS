...
/*
BRCTL_SET_PORT_RELAY与BRCTL_DEL_PORT_RELAY
在内核版本版本linux/if_bridge.h中定义
在此定义，主要是为了让应用模块不依赖内核版本，而导致编译不通过。
*/
#define BRCTL_SET_PORT_RELAY 34
int br_set_portrelay(const char *pszValue,int iEnable)
{
     int iRet;
     char acBuff[17];
     struct ifreq ifr;
     strncpy(ifr.ifr_name, "br0", IFNAMSIZ);
     unsigned long args[4] = { BRCTL_SET_PORT_RELAY, (unsigned long)acBuff, 0,0};
     strncpy(acBuff,pszValue,16);
     args[2] = iEnable;
     ifr.ifr_data = (char *) &args;
     iRet = ioctl(br_socket_fd, SIOCDEVPRIVATE, &ifr);

     return iRet < 0 ? errno : 0;
}
